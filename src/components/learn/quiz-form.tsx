...
import { Check, X, ArrowRight, Sparkle } from "@phosphor-icons/react";
...
function ScoreCircle({ score, passed, size = 180 }: { score: number; passed: boolean; size?: number }) {
  const r = (size - 14) / 2;
  const C = 2 * Math.PI * r;
  ...
  return (
    <div className="relative mx-auto" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <div className="num font-bold text-(--foreground)" style={{ fontSize: size * 0.32, lineHeight: 1, letterSpacing: "-0.02em" }}>
          {score}
          <span className="font-medium text-(--foreground-muted)" style={{ fontSize: size * 0.13 }}>%</span>
        </div>
...
function ResultRow({
  ...
  return (
    <div className="flex gap-3 items-start rounded-[10px] border border-(--border) bg-(--surface) p-3.5">
      <div
      ...
      <div className="min-w-0 flex-1">
        ...
        {exp && (
          <div className="mt-2 flex gap-2 rounded-lg bg-(--info-bg) p-3 text-[13px] leading-relaxed text-(--foreground)">
            <Sparkle size={14} weight="bold" className="mt-0.5 shrink-0 text-(--info)" />
            <span>{exp}</span>
          </div>
        )}
      </div>
...
export function QuizForm({ quiz, courseSlug }: QuizFormProps) {
  ...
  if (result) {
    const resultMap = new Map(result.questionResults.map((r) => [r.questionId, r]));
    return (
      <div className="space-y-6">
        {/* Score card */}
        <div className="rounded-[14px] border border-(--border) bg-(--surface) p-6 text-center md:p-9">
          <ScoreCircle score={result.scorePct} passed={result.passed} size={180} />
          <div
            className="mt-4 text-uism font-semibold uppercase tracking-widest"
            style={{ color: result.passed ? "var(--success)" : "var(--destructive)" }}
          >
          ...
          <p className="text-body text-(--foreground-muted)">
            ตอบถูก{" "}
            <span className="num font-semibold text-(--foreground)">{result.correctCount}</span>{" "}
            จาก{" "}
            <span className="num">{result.totalQuestions}</span> ข้อ
          </p>
        </div>
            ...
            {quiz.questions.map((q, idx) => {
              ...
              return (
                <ResultRow
                  ...
                  q={q.promptMd}
                  exp={!r?.isCorrect ? `คำตอบที่ถูกต้องคือตัวเลือก ${String.fromCharCode(65 + q.choices.findIndex((c) => c.id === r?.correctChoiceId))}` : undefined}
                />
              );
            })}
          </div>
      ...
      <div>
        <div className="mb-2 flex justify-between">
          <span className="text-uism font-semibold text-(--foreground-muted)">
            ข้อ <span className="num text-(--primary)">{currentIndex + 1}</span> จาก{" "}
            <span className="num">{total}</span>
          </span>
          <span className="num text-uism text-(--foreground-muted)">{progressPct}%</span>
        </div>
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  style={{
                    border: isSelected ? "6px solid var(--primary)" : "1.5px solid var(--border-strong)",
                ...
                <span className="text-bodylg num font-medium text-(--foreground)">{c.body}</span>
              </label>
            );
          })}
        </div>
