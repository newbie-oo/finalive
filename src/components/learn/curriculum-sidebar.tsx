...
function StatusIcon({ status, locked }: { status: string; locked: boolean }) {
	if (locked)
		return <LockSimple size={14} className="text-foreground-subtle" />;
	if (status === "completed")
		return <CheckCircle size={14} weight="fill" className="text-success" />;
	if (status === "in_progress")
		return <PlayCircle size={14} weight="fill" className="text-primary" />;
	...
	return (
		<nav className="flex h-full flex-col bg-(--surface)">
			...
			<div className="flex items-center justify-between border-b border-(--border) px-5 py-4">
				<div>
					<h2 className="text-h4" style={{ margin: 0 }}>
						หลักสูตร
					</h2>
					...
					<div key={mod.id} className="mb-4">
						...
						<ul className="space-y-0.5">
							{mod.lessons.map((les) => {
								...
								const passed = les.quizId
									? passedQuizIds?.get(les.quizId)
									: undefined;
								return (
									<Fragment key={les.id}>
										<li>
