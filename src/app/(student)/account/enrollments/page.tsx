...
const statusTone: Record<
	string,
	"success" | "warning" | "review" | "destructive" | "neutral"
> = {
	paid: "success",
...
export default async function EnrollmentsPage() {
	...
	const actionablePendings = pendings.filter(
		(p) =>
			!enrolledCourseSlugs.has(p.courseSlug) &&
								...
								{inProgress.map((e) => (
									<li key={e.enrollmentId}>
										<EnrollmentCard
											...
											coverUrl={coverImageUrl(e.coverStorageKey)}
										...
										<li key={p.pendingId}>
											<Card className="flex h-full flex-col gap-3 overflow-hidden p-0">
												...
												>
													{p.coverStorageKey ? (
														<Image
															src={coverImageUrl(p.coverStorageKey)!}
															alt=""
															fill
