...
const mockCanEditCourse = vi.fn();
vi.mock("@/server/services/course-authz", () => ({
  canEditCourse: mockCanEditCourse,
}));
...
describe("requireCourseAccess", () => {
  beforeEach(() => {
    ...
    mockCanEditCourse.mockReset();
  });

  it("returns course when found and editable", async () => {
    mockGetAdminCourseById.mockResolvedValue({ id: "c1", title: "Course" });
    mockCanEditCourse.mockResolvedValue(true);
  ...
  it("returns forbidden when not editable", async () => {
    mockGetAdminCourseById.mockResolvedValue({ id: "c1", title: "Course" });
    mockCanEditCourse.mockResolvedValue(false);
    const result = await requireCourseAccess(fakeSession, "c1");
