...
export interface CurriculumAdminDeps {
	...
	/** Fast O(1) existence check — avoids fetching the whole curriculum tree. */
...
export function createCurriculumAdminService(
	...
	async function verifyModuleInCourse(
		...
		const exists = await deps.moduleExistsInCourse(moduleId, courseId);
		return exists ? { ok: true } : { ok: false, error: "not_found" };
	}
	...
	async function verifyLessonInCourse(
		...
		const exists = await deps.lessonExistsInCourse(lessonId, courseId);
		return exists ? { ok: true } : { ok: false, error: "not_found" };
	}
