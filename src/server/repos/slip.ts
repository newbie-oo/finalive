...
/* ─── Slip review / upload data access (merged from payments/slip-repo) ─── */
...
export async function countSlipsForPending(pendingId: string): Promise<number> {
	const rows = await db
...
export async function runAcceptTx(
	slipId: string,
	pendingId: string,
	row: SlipReviewRow,
	adminUserId: string,
...
export async function runRejectTx(
	slipId: string,
	pendingId: string,
	reason: RejectReason,
...
export async function getSlipImageMedia(
	slipId: string,
): Promise<
...
export async function finalizeUploadTx(input: {
	mediaId: string;
	pendingId: string;
	userId: string;
