...
describe("isUserEnrolledInCourse", () => {
	it("returns true for an active enrollment", async () => {
		expect(await EnrollmentRepo.hasActive(ENROLLED, COURSE_ID)).toBe(true);
	});

	it("returns false when the user has no enrollment row", async () => {
		expect(await EnrollmentRepo.hasActive(NOT_ENROLLED, COURSE_ID)).toBe(false);
	});

	it("returns false when the only enrollment is cancelled", async () => {
		expect(await EnrollmentRepo.hasActive(CANCELLED, COURSE_ID)).toBe(false);
	});
});
