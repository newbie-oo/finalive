# Static Homepage Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign the homepage to be fully static — remove all database queries, session checks, and dynamic data fetching. Replace with hardcoded marketing content and remove course-progress UI elements that cannot render without user data.

**Architecture:** Convert `src/app/page.tsx` from an async server component with DB/session dependencies to a synchronous, static component with all content hardcoded. Remove the "progress course" card from `HeroVisual` since it requires enrollment data. Keep the existing design system tokens and `PublicShell` layout.

**Tech Stack:** Next.js 16 (App Router), React 19, Tailwind CSS v4, Phosphor Icons (SSR), existing design token system.
...
| File               | Action      | Purpose                                  |
| ------------------ | ----------- | ---------------------------------------- |
| `src/app/page.tsx` | **Rewrite** | Main homepage — static, no data fetching |

---

## Task 1: Rewrite `src/app/page.tsx` as Static Page

**Files:**

- Modify: `src/app/page.tsx`

### Step 1: Remove dynamic export and all server imports

Delete these lines from the top of `src/app/page.tsx`:
...
### Step 2: Replace the default export with a synchronous, static component

Replace the entire `export default async function Home()` with this static version:
          ...
          <div className="grid items-center gap-10 md:grid-cols-2 md:gap-16">
            ...
            <div>
              ...
              <p className="mt-5 text-body text-(--foreground-muted)">
                ...
                เคยดำรงตำแหน่ง VP Investment ในบริษัทจัดการกองทุนชั้นนำ
                และถือใบอนุญาต CFA Charterholder
          ...
          <div className="flex flex-col items-start justify-between gap-5 md:flex-row md:items-center md:gap-8">
            <div>
              ...
              <p className="mt-2.5 max-w-[540px] text-bodylg text-white/80">
                ลงทะเบียนฟรีวันนี้ เริ่มจากคอร์สตัวอย่าง —
                ดูได้ทันทีไม่ต้องชำระเงิน
              </p>
            </div>
...
### Step 3: Add the new `StaticCourseCard` component

Add this component inside `src/app/page.tsx`, after the `Home` component and before the existing helper components:
...
Add `Users` to the Phosphor icons import at the top of the file:
...
### Step 4: Update `HeroVisual` — remove progress card

Replace the existing `HeroVisual` function with this static version that removes the "progress course" card (which requires enrollment data):
...
### Step 5: Keep the helper components

The existing `Eyebrow`, `TrustStat` helper components remain unchanged. Keep them at the bottom of the file.

### Step 6: Verify the file has no remaining dynamic imports

Run this grep to confirm no server-only imports remain:

```bash
grep -n "@/server/" src/app/page.tsx || echo "No server imports found — OK"
grep -n "@/components/course/course-card" src/app/page.tsx || echo "No CourseCard import found — OK"
grep -n "force-dynamic" src/app/page.tsx || echo "No force-dynamic found — OK"
grep -n "async function" src/app/page.tsx || echo "No async function found — OK"
```

Expected output: all lines should print `"... — OK"`.
...
### Step 8: Build the project to verify static generation

```bash
npm run build
```

Expected: Build succeeds. In the build output, look for confirmation that the homepage is statically generated (no `λ` lambda marker on `/`).

### Step 9: Commit

```bash
git add src/app/page.tsx
git commit -m "refactor(homepage): convert to fully static page

- Remove all DB queries (listFeaturedCourses, getPublicHomeStats)
- Remove session check (getSession)
- Remove force-dynamic export
- Replace dynamic CourseCard with StaticCourseCard (3 hardcoded courses)
- Replace dynamic stats with static numbers
- Remove session-dependent conditional CTAs
- Remove progress card from HeroVisual (requires enrollment data)
- Add course library visual card instead of progress card"
```
...
| Requirement                      | Task                                                         |
| -------------------------------- | ------------------------------------------------------------ |
| No DB queries at render time     | Step 1 — removed `listFeaturedCourses`, `getPublicHomeStats` |
| No session checks at render time | Step 1 — removed `getSession`                                |
| No `force-dynamic` export        | Step 1 — removed `export const dynamic = "force-dynamic"`    |
| Static course showcase           | Step 2 — 3 hardcoded `StaticCourseCard` instances            |
| Static stats                     | Step 2 — hardcoded "12,500+", "41", "180+"                   |
| Static CTAs (no login condition) | Step 2 — single CTA set, always shows "ลงทะเบียนฟรี"         |
| No course progress display       | Step 4 — removed progress card from `HeroVisual`             |
| Static visual cards in hero      | Step 4 — library card + cert card (no enrollment data)       |
...
- All type signatures are explicit

## Type Consistency Check

- `StaticCourseCard` props use `string` for all display fields — consistent
- Tag tone union matches Tailwind class map keys — consistent
- `Users` icon added to existing Phosphor import block — consistent
- Helper components (`Eyebrow`, `TrustStat`) unchanged — consistent

---

**Plan complete and saved to `docs/superpowers/plans/2025-05-06-static-homepage-redesign.md`.**
